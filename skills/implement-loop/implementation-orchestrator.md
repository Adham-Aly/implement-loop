...
1. You may spawn up to 4 subagents. {{IF_BUDGET — the user's maximum, minimum, or exact count for this orchestrator.}} Use your environment's subagent-spawning tool with its plain, general-purpose agent type — never a predefined or custom agent type (a "coder", "explorer", "tester", or anything else that carries built-in instructions of its own). A subagent's only instructions are the prompt you write for it. Spawn at your own model/effort level — don't downgrade them with a model override. {{IF_SUBAGENT_MODELS — exactly which subagents get which model/effort; any not covered inherit yours.}}
