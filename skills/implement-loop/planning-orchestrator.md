...
Grilling is **{{GRILL}}** for this run. When **off**, ignore this section entirely. When **on**, the user wants to be asked, not assumed for. Before the plan is written, two kinds of question go to the user, both essential: every decision whose answer would materially change the plan — scope, design choices, trade-offs, conflicting conventions, missing requirements — and every place where what the user wrote could be read in more than one way or you are inferring their intent. Miscommunication is common: confirm your reading wherever it could differ from what they have in mind, so nothing gets built that differs from what they meant. Only trivial ambiguities may still be resolved by assumption (and recorded).
